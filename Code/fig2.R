...
score_table_clustering <- scores_w_pheno%>%
  filter(timepoint == "baseline", site != "imx")%>%
...
column_order = c("mars3_score", "mod4_score","adaptive_score", "yao_IA_score", "coagulopathic_score", "yao_IC_score", "mars1_score", "wong_score","mod3_score", "mars4_score","mod2_score", "inflammopathic_score", "yao_IN_score", "mars2_score", "mod1_score","davenport_SRSq","cano_SRSq")

site = filter(scores_w_pheno, site != "imx", timepoint == "baseline")$site

subspace_heatmap <- Heatmap(score_table_matrix,   show_row_dend = FALSE, column_dend_reorder = F,right_annotation = rowAnnotation(
  df = data.frame(site),
  col = list(foo = c(display.brewer.pal(n = 8, name = 'Dark2'))),
  show_legend = TRUE,
  show_annotation_name = FALSE
),clustering_method_columns = "ward.D2", column_split = 4, column_title = NULL,top_annotation = HeatmapAnnotation(foo = anno_block(gp = gpar(fill = c("#40A1FF","#80C684","#A781BA","#F47B00")))), column_labels = c("MARS 3","SoM Mod 4","Adaptive","Yao Adaptive", "Coagulopathic","Yao Coagulopathic","MARS 1", "Wong Score", "SoM Mod 3"," MARS 4","SoM Mod 2", "Inflammopathic","Yao Innate","MARS 2","SoM Mod 1"," Davenport SRS"," Cano-Gamez SRS"), use_raster = FALSE
)
...
#generates edges from correlation matrix - selected >median absolute spearman correlations
threshold <- 0.47
...
E(graph_data)$width <- case_when(E(graph_data)$V3 <0.5 ~1, 
                                 E(graph_data)$V3 >=0.5 & E(graph_data)$V3 <0.7 ~2,
                                 E(graph_data)$V3 >=0.7 & E(graph_data)$V3 <0.9 ~4,
                                 E(graph_data)$V3 >=0.9 ~5)
