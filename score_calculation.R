...
annotation = biomaRt::select(org.Hs.eg.db, keys = rownames(expr), columns = "ENSEMBL", keytype = "SYMBOL")%>%
  distinct(SYMBOL, .keep_all = TRUE)

ensembl_expr <- data

ensembl_expr <- left_join(annotation,ensembl_expr,  by = "SYMBOL")%>%
  filter(!is.na(ENSEMBL))%>%
  dplyr::select(SYMBOL,ENSEMBL, everything()) 

rownames(ensembl_expr) <- ensembl_expr$ENSEMBL

#transposes data frame to have the ensembl as column names for input into sepstratifier 
sepstratifier_expr <- t(ensembl_expr[,-c(1,2)])
...
sweeney_table  <- sweeney_table  %>%
  dplyr::rename(inflammopathic_score = score1,
         adaptive_score = score2,
         coagulopathic_score = score3)

##################
#Davenport and Cano-Gamez Scores using Sepstratifier
davenport_scores <- stratifyPatients(sepstratifier_expr,gene_set = "davenport", k = 20) #depending on dataset, may need to adjust k

cano_scores <- stratifyPatients(sepstratifier_expr,gene_set = "extended", k = 20) #depending on dataset, may need to adjust k
...
wong_table <- (wong_int2 - wong_int2[,"geomean"])^2 %>%
  mutate(wong_score = rowSums(.)/1000000,
         accession = rownames(wong_int2))%>%
  dplyr::select("accession","wong_score")
...
mars1_score <- unlist(mars1_score)
mars1_table<-data.table(names(mars1_score),mars1_score) %>%
  setnames("V1", "accession")

mars2_score = getGeneScores_mars(hgnc_expr,pos=mars2_up,mars2_down)
mars2_score <- unlist(mars2_score)
mars2_table<-data.table(names(mars2_score),mars2_score) %>%
  setnames("V1", "accession")

mars3_score = getGeneScores_mars(hgnc_expr,pos=mars3_up,mars3_down)
mars3_score <- unlist(mars3_score)
mars3_table<-data.table(names(mars3_score),mars3_score) %>%
  setnames("V1", "accession")

mars4_score = getGeneScores_mars(hgnc_expr,pos=mars4_up,mars4_down)
mars4_score <- unlist(mars4_score)
mars4_table<-data.table(names(mars4_score),mars4_score) %>%
  setnames("V1", "accession")
