...
srt_final <- readRDS("single_cell_scores_seurat.RDS")
...
platelet_genes <- c("IGF2BP2","NAPA","PDCD10","STX1A")
...
mat.expr = as.matrix(srt_scores[,-1])
rownames(mat.expr) = srt_scores$celltype
...
srt_matrix_int1= t(as.matrix(srt_cell_prop))
colnames(srt_matrix_int1) = "mars1_score"

srt_matrix_int2 <- data.frame(srt_matrix_int1)%>%
...
mat.N = as.matrix(srt_matrix_int2)
mat.N = t(mat.N) 
...
pht = Heatmap(mat.expr,
              ...
              left_annotation = rowAnnotation(foo = anno_block(gp = gpar(fill = c("#A781BA","#F3766E","#1CBDC2","#7CAF41")),
                                                               labels = c("", "", "", ""), 
...
mat.expr = as.matrix(srt_genes[,-1])
rownames(mat.expr) = srt_genes$celltype_modified
mat.expr = scale(mat.expr)
...
pht = Heatmap(mat.expr,
              ...
              show_column_names = T,
              row_names_side = "left",
              left_annotation = rowAnnotation(df = data.frame(Category), col = list(Category = c("Detrimental" = "orange", "Protective" = "mediumaquamarine")), show_legend = F, show_annotation_name = F),
              cluster_rows = F,show_row_dend = F,show_column_dend = F,row_dend_side = "left",column_dend_side = "top",
              clustering_method_rows = "ward.D2",clustering_method_columns  = "ward.D2",
              clustering_distance_columns  = "euclidean", clustering_distance_rows  = "euclidean",
